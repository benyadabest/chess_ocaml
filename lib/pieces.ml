...
let legal_moves piece (row, col) =
  ...
  | King ->
      ...
      in
      List.filter
        (fun (r, c) -> r >= 0 && r < 8 && c >= 0 && c < 8)
        (List.map (fun (dr, dc) -> (row + dr, col + dc)) king_moves)
  | Queen ->
      List.append
        (generate_rook_moves (row, col) 8)
        (generate_diagonal_moves (row, col) 8)
  | Rook -> generate_rook_moves (row, col) 8
  | Bishop -> generate_diagonal_moves (row, col) 8
  | Knight -> generate_knight_moves (row, col)
  | Pawn ->
      if piece.color = White then
        if row = 1 then [ (row + 1, col); (row + 2, col) ]
        else [ (row + 1, col) ]
      else if row = 6 then [ (row - 1, col); (row - 2, col) ]
      else [ (row - 1, col) ]
